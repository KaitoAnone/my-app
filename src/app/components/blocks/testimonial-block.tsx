...
export interface TestimonialBlock {
  ...
  photo: {
    ...
    name: string;
    url: string;
    width?: number;  // เพิ่มค่า width ที่อาจจะได้รับจาก API
    height?: number; // เพิ่มค่า height ที่อาจจะได้รับจาก API
  };
}
...
export function TestimonialBlock({ block }: { block: TestimonialBlock }) {
  ...
  // ใช้ขนาดจริงของภาพจากข้อมูลที่ได้รับจาก API (ถ้ามี)
  const imageWidth = block?.photo?.width || 800; // กำหนดค่า default หากไม่มีข้อมูล
  const imageHeight = block?.photo?.height || 600; // กำหนดค่า default หากไม่มีข้อมูล
      ...
      <div className="grid grid-cols-1 md:grid-cols-3">
        <div className="relative h-64 md:h-full col-span-1">
          <Image
            ...
            width={imageWidth}  // ใช้ width จากข้อมูล
            height={imageHeight} // ใช้ height จากข้อมูล
            objectFit="contain" // ปรับให้ภาพแสดงตามขนาดจริง
